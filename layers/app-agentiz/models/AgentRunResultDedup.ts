...
@Table({
  tableName: 'agentiz_run_result_dedup',
  timestamps: true,
  indexes: [{ unique: true, fields: ['jobId', 'attempt', 'resultId'], name: 'agentiz_run_result_dedup_unique' }],
})
export class AgentRunResultDedup extends Model<InferAttributes<AgentRunResultDedup>, InferCreationAttributes<AgentRunResultDedup>> {
  @Default(() => randomUUID())
