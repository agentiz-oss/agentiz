...
export const PIPELINE_SPEC_RULES: readonly string[] = [
  ...
  'stages[].verdict (default false) adds an instruction to that stage\'s prompt asking for a `AGENTIZ_VERDICT: pass` or `AGENTIZ_VERDICT: fail — reason` marker, parsed from the stage\'s own output only (never from other stages\' text) into AgentRun.verdict/verdictReason. Absent means the pre-existing behaviour: nothing is added to the prompt and AgentRun.verdict stays null, same as when the marker was asked for but never produced.',
  ...
  'A pipeline spec is an entity of its project and never moves to another one: changing PipelineSpec.projectId is refused. A worker directory can be bound the same way (projectId on the declared workspace, via agentiz.manageWorker {operation:"setWorkspaces"}), and then only that project\'s specs may name it — by workspaceKey or by its path — checked both when the spec is saved and when a run is queued. An unbound directory stays shared.',
