...
export let webappProcess: any = null;
 ...
 */
export const startWebappDevServer = async (appManager: AppManager): Promise<void> => {
  if (process.env.NODE_ENV !== 'development' || process.env.ENABLE_WEBAPP === 'false') {
    return;
  }
  ...
  if (!portInUse) {
    ...
    webappProcess.on('error', (err: Error) => {
 ...
 */
export const setupWebappProxy = (appManager: AppManager): void => {
  if (process.env.NODE_ENV === 'development') {
    // Proxy / to webapp server, excluding API routes
    appManager.app.use((req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    // Fallback for SPA
    appManager.app.get('*', (req: express.Request, res: express.Response, next: express.NextFunction) => {
      if (req.path.startsWith('/graphql') ||
          req.path.startsWith('/health') ||
          req.path.startsWith('/switch') ||
          req.path.startsWith('/api') ||
 ...
 */
export const stopWebappDevServer = (signal: string = 'SIGTERM'): void => {
