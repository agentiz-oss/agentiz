...
> **Обновление 2026-08-07.** Разделы про клонирование, выдачу секретов и сбор диффа реализованы.
> Отличия от написанного ниже: работа разнесена по трём модулям — `repository.py` (clone/checkout и
> безопасная подача кредов), `changes.py` (патч и файловые операции), `redaction.py` (маскирование);
> секреты подаются агенту не через `Conversation.secrets`, а только в git — воркеру они нужны для
> клонирования, и чем меньше кода их видит, тем лучше. Стадия `runtime.mode: docker` в job'е с
> репозиторием пока отклоняется: доставка чекаута внутрь `DockerWorkspace` не проверена на реальном
> API. Подробности — `.ai-notes/multi-repo-oauth/06-worker-checkout-and-diff.md`.
