...
- Что собирается, решает `harness_usage.COLLECTORS` — по одному сборщику на harness-ключ.
  `claude` читает токен из `~/.claude/.credentials.json` (или `CLAUDE_CONFIG_DIR`) и передаёт
  ответ usage-эндпоинта как есть. `codex` запускает установленный CLI как
  `codex app-server --stdio`, делает JSON-RPC `initialize` и `account/rateLimits/read`, после чего
  передаёт исходный результат как есть. Проверенный минимум Codex CLI — **0.153.4**; более старый
  app-server просто не создаст sample и напишет короткое предупреждение, не останавливая worker.
- Для Codex нужен `codex login` в ChatGPT-аккаунт под **тем же Unix-пользователем**, что запускает
  `agentiz-worker`; CLI сам читает `~/.codex` и worker никогда не читает/не передаёт OAuth-token.
  `AGENTIZ_CODEX_BIN` задаёт точный путь к CLI, а `AGENTIZ_CODEX_USAGE_TIMEOUT_SEC` ограничивает
  весь app-server запрос (1–60 секунд, по умолчанию 20). Путь из `PATH` и обычные user-install
  каталоги также попадают в systemd unit при `install-service`.
- Воркер ничего не интерпретирует. Названия окон и полей — словарь провайдера, разбирают слои
  `app-agentiz-claude-limits` и `app-agentiz-codex-limits` на сервере; так изменившийся формат
  чинится в одном месте.
...
- API-key режим Codex (`OPENAI_API_KEY`/`CODEX_API_KEY`) не имеет ChatGPT subscription windows:
  он не должен использоваться для этой телеметрии. Codex также не входит в `POKERS`: серверный
  `openWindow` не вызывает искусственный запрос и не расходует quota ради побудки.
