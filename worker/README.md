...
- Просроченный токен **пропускается, а не обновляется**: хранилище кредов принадлежит CLI, и
  переписывать его наперегонки с ним нельзя. CLI сам обновит токен, следующий тик его подхватит.
